...
class FullSegmentInfo {
 public:
  ...
  void computeLineInfo() {
      float dx = endpoints(0) - endpoints(2);
      float dy = endpoints(1) - endpoints(3);
     length = sqrt(dx * dx + dy * dy);
     line_angle = std::atan2(dy, dx);
//     if (line_angle < 0)
//         line_angle += M_PI;
     center_x = 0.5 * (endpoints(0) + endpoints(2));
     center_y = 0.5 * (endpoints(1) + endpoints(3));
  }
  ...
  float length;
  float line_angle; //std::atan2   [-pi, pi]
