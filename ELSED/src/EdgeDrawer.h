...
class EdgeDrawer {
 public:
  ...
  void init(const LineDetectionExtraInfoPtr &gradientInfo, cv::Mat &edgeImage, cv::Mat &nmsImage);

  void drawEdgeInBothDirections(uint8_t direction, Pixel anchor);

  inline std::vector<FullSegmentInfo> &getDetectedFullSegments() {
    return segments;
  }
  ...
  void setOnlyInsideNMSRegion(const bool& inside) {only_inside_nms = inside;}
  void setNMSImage(uint8_t *img) {nmsImg = img;}
  void setNMSID(const int& id) {NMS_id = id;}
