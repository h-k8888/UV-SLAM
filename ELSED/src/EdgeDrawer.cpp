...
EdgeDrawer::EdgeDrawer(const LineDetectionExtraInfoPtr &gradientInfo,
  ...
  init(gradientInfo, edgeImage, nmsImage);
}

void EdgeDrawer::init(const LineDetectionExtraInfoPtr &gradientInfo, cv::Mat &edgeImage,  cv::Mat &nmsImage) {
  ...
  assert(!nmsImage.empty());
  assert(nmsImage.type() == CV_8UC1);
  assert(nmsImage.size() == gradientInfo->gImg.size());
  nmsImg = nmsImage.ptr<uint8_t>();
...
//         << "\n\t- Initial Pixels: " << branch.pixels;
    ...
    lastPx = calcLastPixelWithDirection(px, direction); //下一个方向

    gradDir = (direction == UPM_DOWN || direction == UPM_UP) ? UPM_EDGE_VERTICAL : UPM_EDGE_HORIZONTAL;

    localSegInitialized = false;
    while (outliersList.size() <= UPM_MAX_OUTLIERS_TH) { //当外点数量未超限, 不断拓展

        //3X3邻域内，返回下一个像素是否有梯度，更新px和lastPx
      ...
      //如果新像素是其他内点、跳跃点、边点，inlierOverwritten记为true，跳出循环
      inlierOverwritten = edgeImg[indexInArray] == UPM_ED_SEGMENT_INLIER_PX ||
        ...
        if (only_inside_nms){
            if(nmsImg[indexInArray] != UPM_ED_NMS_PX )
                break;
        }
        else if (nmsImg[indexInArray] == UPM_ED_NMS_PX)
        {
      ...
      if (segment) {
        ...
        if (pixels.size() - lastChekedPxIdx >= minLineLength) {
          ...
          if (localSegInitialized) {
            ...
            localSegment->init(pixels, lastChekedPxIdx); //根据pixels初始化直线
          }
...
void EdgeDrawer::drawEdgeInBothDirections(uint8_t direction, Pixel anchor) {
  ...
  if (gradImg[anchor.y * imageWidth + anchor.x] <= 0)
      return;
  assert(gradImg && gradImg[anchor.y * imageWidth + anchor.x] > 0);

  ImageEdge anchorPixels, newBranchPixels; //std::vector<Pixel>
  edgeImg[anchor.y * imageWidth + anchor.x] = UPM_ED_ANCHOR_PIXEL;//设定输入点为 UPM_ED_ANCHOR_PIXEL
